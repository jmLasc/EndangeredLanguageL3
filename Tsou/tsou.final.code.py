...
def remove_star(lines):
    return [line for line in lines if "★" not in line and not re.search(r'(\bhttps://e-dictionary.ilrdf.org.tw\b)|([\u4e00-\u9fff]*鄒語[\u4e00-\u9fff]*)', line)]
...
#     # Detect and add new lines after the regular expression match
#     new_data = re.sub(regex_pattern, r"\0\n", data)

#     with open(output_file, "w", encoding="utf-8") as file:
#         file.write(new_data)


def add_newlines(input_file, output_file, regex_pattern_1):
    with open(input_file, "r", encoding="utf-8") as file:
        data = file.read()

    # Apply each regex pattern and add new lines after the matches
    for regex_pattern in regex_pattern_1:
        data = re.sub(regex_pattern, r"\0\n", data)
...
def transcribe_lines():
    with open("tsou_stripped(3.0).txt", "r") as input_file:
    ...
    with open("tsou_stripped_final.txt", "w") as output_file:
...
#4.26 code
read_pdf_text('tsou.pdf', 'tsou_stripped.txt')

#6.9 code 
read_text_file('tsou_stripped.txt', 'tsou_stripped(1.0).txt')

#6.12 & 6.13 code 
input_file = "tsou_stripped(1.0).txt"
output_file_1 = "tsou_stripped(2.0).txt"
output_file_2 = "tsou_stripped(3.0).txt"
# regex_pattern_1 = r"([\u4e00-\u9fff]+\. )|([\u4e00-\u9fff]+ \.)|([\u4e00-\u9fff]+\S\.)|[\u4e00-\u9fff]+\S \.|[\u4e00-\u9fff]+\S \. |[\u4e00-\u9fff]+\S\. | [\u4e00-\u9fff]+ ."
# regex_pattern_1 = r"^(?:\d+\. |.*?[A-Za-z]+\. |['\"]| [\u4e00-\u9fff]+[. ])|([\u4e00-\u9fff]+[.])"
# regex_pattern_1 = r" ([\u4e00-\u9fff]+ .) | ([\u4e00-\u9fff]\.) "
#regex_pattern_1 = r"([\u4e00-\u9fff]\. )| (\)\.\s) | ([\u4e00-\u9fff] \. ) | (….. ) | (…. )"
regex_pattern_1 = [
    r"([\u4e00-\u9fff]\. )",
    r"(\)\.\s)",
    r"([\u4e00-\u9fff] \. )",
