...
def remove_star(lines):
    return [line for line in lines if "★" not in line and not re.search(r'(\bhttps://e-dictionary.ilrdf.org.tw\b)|([\u4e00-\u9fff]*賽夏語[\u4e00-\u9fff]*)', line)]
...
def transcribe_lines():
    with open("saisiyat_stripped(3.0).txt", "r") as input_file:
    ...
    with open("saisiyat_stripped_final.txt", "w") as output_file:
        for line in output_lines:
            output_file.write(line + "\n")

#4.26 code
read_pdf_text('saisiyat.pdf', 'saisiyat_stripped.txt')

#6.9 code 
read_text_file('saisiyat_stripped.txt', 'saisiyat_stripped(1.0).txt')

#6.12 & 6.13 code 
input_file = "saisiyat_stripped(1.0).txt"
output_file_1 = "saisiyat_stripped(2.0).txt"
output_file_2 = "saisiyat_stripped(3.0).txt"
