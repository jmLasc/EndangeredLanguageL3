...
def remove_star(lines):
    return [line for line in lines if "★" not in line and not re.search(r'(\bhttps://e-dictionary.ilrdf.org.tw\b)|([\u4e00-\u9fff]*噶瑪蘭語[\u4e00-\u9fff]*)', line)]
...
##    with open("kavalan_stripped(3.0).txt", "r") as input_file:
...
##        output_lines.append(current_line.strip())
##
##    with open("kavalan_stripped_final.txt", "w") as output_file:
...
##        output_lines.append(current_line.strip())
##
##    with open("puyuma_stripped_final.txt", "w") as output_file:
##        for line in output_lines:
##            output_file.write(line + "\n")
...
#4.26 code
read_pdf_text('kavalan.pdf', 'kavalan_stripped.txt')

#6.9 code 
read_text_file('kavalan_stripped.txt', 'kavalan_stripped(1.0).txt')

#6.12 & 6.13 code 
input_file = "kavalan_stripped(1.0).txt"
output_file_1 = "kavalan_stripped(2.0).txt"
output_file_2 = "kavalan_stripped(3.0).txt"
regex_pattern_1 = r"([\u4e00-\u9fff]+\. )|([\u4e00-\u9fff]+ \.)|([\u4e00-\u9fff]+\S\.)|([\u4e00-\u9fff]+\S \.)|([\u4e00-\u9fff]+\S \. )|([\u4e00-\u9fff]+\S\. )|(\s\)\.\s)|((\s\)\s\.\s))"
