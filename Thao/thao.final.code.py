...
def remove_star(lines):
    return [line for line in lines if "★" not in line and not re.search(r'(\bhttps://e-dictionary.ilrdf.org.tw\b)|([\u4e00-\u9fff]*邵語[\u4e00-\u9fff]*)', line)]
...
def transcribe_lines():
    with open("thao_stripped(3.0).txt", "r") as input_file:
    ...
    with open("thao_stripped_final.txt", "w") as output_file:
        for line in output_lines:
            output_file.write(line + "\n")

#4.26 code
read_pdf_text('thao.pdf', 'thao_stripped.txt')

#6.9 code 
read_text_file('thao_stripped.txt', 'thao_stripped(1.0).txt')
